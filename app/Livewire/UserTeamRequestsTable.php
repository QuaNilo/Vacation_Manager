...
class UserTeamRequestsTable extends Component implements HasForms, HasTable
{
    ...
    {
        $newModel = new UserTeamRequests();
        return $table
            ->query(UserTeamRequests::query())
            ->columns([
            ...
            TextColumn::make("team_id")
                ->label($newModel->getAttributeLabel("team_id"))
                ->sortable()
                ->toggleable()
                ->searchable(),
            ...
            ->actions([
                Action::make('edit')
                ->label(__('Update'))
                ->url(fn (UserTeamRequests $record): string => route('user-team-requests.edit', ['user_team_requests' => $record]))
            ...
            ->defaultSort('id', 'desc')
            ->recordUrl(
                fn (Model $record): string => route('user-team-requests.show', ['user_team_requests' => $record]),
            )
            //->striped()
    ...
    {
        return view('user_team_requests.table');
    }
}
