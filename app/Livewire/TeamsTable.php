    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ...
            TextColumn::make("category")
                ...
                ->toggleable()
                ->searchable(),
            TextColumn::make("members_max_vacation_days")
                ->label($newModel->getAttributeLabel("members_max_vacation_days"))
                ->sortable()
                ->toggleable()
                ->searchable(),
            TextColumn::make("members_max_on_vacation")
                ->label($newModel->getAttributeLabel("members_max_on_vacation"))
                ->sortable()
                ->toggleable()
                ->searchable(),
            TextColumn::make("members_vacation_days_regen_monthly")
                ->label($newModel->getAttributeLabel("members_vacation_days_regen_monthly"))
                ->sortable()
                ->toggleable()
                ->searchable(),
            ...
            ->actions([
                Action::make('edit')
                ->label(__('Update'))
                ->url(fn (Team $record): string => route('teams.edit', ['team' => $record]))
            ...
            ->defaultSort('id', 'desc')
            ->recordUrl(
                fn (Model $record): string => route('teams.show', ['team' => $record]),
            )
            //->striped()
