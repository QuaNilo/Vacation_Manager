...
class TableFieldsGenerator
{
    ...
    public function __construct($tableName, $ignoredFields, $connection = '')
    {
        ...
        /*Inicio do codigo adicionado para suportar laravel 11*/
        $connectionParams = [
            'dbname'   => config('database.connections.mysql.database'),
            'user'     => config('database.connections.mysql.username'),
            'password' => config('database.connections.mysql.password'),
            'host'     => config('database.connections.mysql.host'),
            'driver'   => 'pdo_mysql', // Change this according to your database driver
            // Add any other necessary configuration parameters here
        ];
        $dbalConnection = DriverManager::getConnection($connectionParams);
        $this->schemaManager = $dbalConnection->createSchemaManager();
        /*Fim do codigo adicionado para suportar laravel 11*/
        /*if (!empty($connection)) {
