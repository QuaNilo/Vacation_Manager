...
<!-- BEGIN: Top Bar -->
<div @class([
    'h-[70px] md:h-[65px] z-[50] border-b border-white/[0.08] mt-12 md:mt-0 -mx-5 sm:-mx-8 md:-mx-0 px-3 md:border-b-0 relative md:fixed md:inset-x-0 md:top-0 sm:px-8 md:px-10 md:pt-10 md:bg-gradient-to-b md:from-slate-100 md:to-transparent dark:md:from-darkmode-700',
    ...
    <div class="flex h-full items-center">
        ...
        >
            ...
            <span @class([
                ...
                {{ auth()->user()->company()->first()->name ?? 'Vacation Manager' }}
            </span>
        </a>
        <!-- END: Logo -->
        @section('breadcrumbs')
        ...
        <x-base.menu>
            ...
            @else
                <x-base.menu.button class="intro-x block h-8 w-8 scale-110 overflow-hidden rounded-full shadow-lg">
                    <span class="px-3 py-3 rounded-full bg-slate-300 text-slate-500 dark:bg-darkmode-800 dark:text-slate-300">{{ Str::substr(auth()->user()->name, 0,1) }}</span>
                </x-base.menu.button>
            @endif
            <x-base.menu.items
